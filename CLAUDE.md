...
- **RLS on `_prisma_migrations`**: this internal Prisma table (no business data) never has RLS enabled — `20260729140700_enable_rls` excludes it, do not add it back. Reason: it's Prisma's own bookkeeping table, and RLS on it breaks `prisma migrate dev`'s local shadow-database replay (P3006/P1014). `migrate deploy`, used by CI/e2e, is unaffected either way.
